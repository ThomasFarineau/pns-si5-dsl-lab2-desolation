...
export function generateJson(model: Model, filePath: string, destination: string | undefined): string {
    ...
    function extractPart(str: string): { note: string, accidental: string, octave: number, duration: string } {
    ...
    function matchNote(str: string): string | null {
        const regex = /([A-Zac-z]{1,3})([b#]?)(\d)(.)/;
        const match = str.match(regex);
    ...
    fileNode.append(JSON.stringify(newModel, null, 2))
